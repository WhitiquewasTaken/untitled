...
class SAM(nn.Module):
    def __init__(self, n_feat, kernel_size=3, bias=True):
        ...
        self.conv2 = conv(n_feat, 2, kernel_size, bias=bias)  # 修改：输出2通道光流
        self.conv3 = conv(2, n_feat, kernel_size, bias=bias)  # 修改：输入2通道

    def forward(self, x, x_img):
        x1 = self.conv1(x)
        # 输入图像是3通道，我们需要处理这个不匹配
        if x_img.shape[1] == 3:
            # 将3通道RGB转换为2通道（取前2个通道）
            x_img_2ch = x_img[:, :2, :, :]
        else:
            x_img_2ch = x_img

        img = self.conv2(x) + x_img_2ch  # 输出2通道光流
...
class UNet(nn.Module):
    def __init__(self, block=ConvBlock,dim=32):
        ...
        self.conv10 = nn.Conv2d(dim, 2, kernel_size=3, stride=1, padding=1)  # 修改：输出2通道光流
...
class OutputProj(nn.Module):
    def __init__(self, in_channel=64, out_channel=2, kernel_size=3, stride=1, norm_layer=None, act_layer=None):
        super().__init__()
        self.proj = nn.Sequential(
            nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=stride, padding=kernel_size//2),  # 输出2通道
        )
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=2, dd_in=3,
        ...
        # ========== 新增：初始估计分支 ==========
        self.initial_flow = nn.Conv2d(dd_in, in_chans, kernel_size=3, padding=1, bias=True)

        # 权重放大5倍（关键！）
        with torch.no_grad():
            self.initial_flow.weight.data.mul_(5.0)
            if self.initial_flow.bias is not None:
                self.initial_flow.bias.data.mul_(5.0)
            print(f"initial_flow 权重已放大5倍，初始范数: {self.initial_flow.weight.norm().item():.4f}")

        # 可学习的缩放参数
        self.init_scale = nn.Parameter(torch.ones(1) * 1.0)
        self.residual_scale = nn.Parameter(torch.ones(1) * 1.0)
        # ======================================
        ...
        self.output_proj = OutputProj(in_channel=2 * embed_dim, out_channel=in_chans, kernel_size=3, stride=1)  # 输出2通道
    ...
    def forward(self, x, mask=None, save_flow=False, save_dir="", filename=""):
        # ========== 新增：初始估计（带可学习缩放） ==========
        initial_flow = self.initial_flow(x) * self.init_scale
        # ==================================================
        ...
        # Output Projection（得到残差）
        flow_residual = self.output_proj(deconv3)  # [B, 2, H, W]

        # ========== 新增：限制残差范围 ==========
        flow_residual = torch.tanh(flow_residual) * 2.0 * self.residual_scale
        # ========================================

        # 最终光流 = 初始估计 + 残差
        final_flow = initial_flow + flow_residual
    ...
    def save_flow_csv(self, flow, save_dir, filename_prefix):
        """将光流保存为CSV文件"""
        import numpy as np
        import os
        ...
        u_path = os.path.join(save_dir, f"{filename_prefix}_flowx.csv")
        v_path = os.path.join(save_dir, f"{filename_prefix}_flowy.csv")

        np.savetxt(u_path, u_flow, delimiter=',', fmt='%.6f')
        np.savetxt(v_path, v_flow, delimiter=',', fmt='%.6f')


class Uformer_Flow(Uformer):
    """专门用于光流估计的Uformer版本"""

    def __init__(self, img_size=256, embed_dim=32,
    ...
    def forward(self, x, save_flow=False, save_dir="", filename=""):
        # 调用父类的forward，只返回光流
        return super().forward(x, save_flow=save_flow, save_dir=save_dir, filename=filename)
